...
int(timer_test_read_config)(uint8_t timer, enum timer_status_field field) {
  ...
  if (timer < 0 || timer > 2) { return 1; } //you just have 3 timers
  uint8_t st; //status -> control word
...
//Definir a frequência dos timers
int(timer_test_time_base)(uint8_t timer, uint32_t freq) {
  
  if (timer < 0 || timer > 2 || timer_set_frequency(timer, freq)) { return 1; }
  return 0;
}
...
int(timer_test_int)(uint8_t time) {
  ...
  int ipc_status, r;
  message msg;
  timer_subscribe_int(&bit_no); //Subscrive a notification of interrupt
